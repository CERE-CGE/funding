...
As mentioned in the section 1, a computable general equilibrium model will be used for the purpose of our research. A CGE model are based on microeconomic foundation and theoretically consistent. It can simulate behavior of the entire economy, not just the parts of the economy that are of interest. A CGE model is also appropriate when a policy in one sector expected to induce changes in other sectors of the economy and feedback is expected from those sectors to other sectors. These models are also useful for analyzing changes in sectoral output, prices, endowments and trade as well as changes in national efficiency (welfare) consequent to policy changes.

In a CGE model, each sector and consumer optimizes their production and consumption decision at different stages of production and utility tree within their resource and budgetary constraints respectively. These models also ensure the balance of income and expenditure of the economic agents, market clearing conditions (supply is equal to demand) and zero profit conditions (sum of inputs is equal to output) for each sector(commodity). 
